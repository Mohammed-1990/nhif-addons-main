...
class DetailedExpensesSynthesisReport(models.AbstractModel):
	_name = 'report.kamil_accounting_reports.detail_exp_temp'
	...
	def _get_report_values(self, docids, data=None):
		...
		for company_id in self.env['res.company'].sudo().search([('id','in',company_ids)]):
			...
			for account in accounts_names_codes:
				...
				self._cr.execute("select COALESCE(sum(COALESCE( l.debit, 0 )), 0 ) - COALESCE(sum(COALESCE( l.credit, 0 )), 0) from account_move_line l inner join account_account a on l.account_id = a.id where a.code like '" + account['account_code'] + '%' + "' and l.date >= '" + str(date_from) + "' and l.date <= '" + str(date_to) + "' and l.company_id = " + str(company_id.id) )
