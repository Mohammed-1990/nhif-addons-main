...
class CompareRevenuesCompaniesReport(models.AbstractModel):
	_name = 'report.kamil_accounting_reports.compare_companies_template'
	...
	def _get_report_values(self, docids, data=None):
		...
		for company_id in self.env['res.company'].sudo().search([]):
			...
			self._cr.execute("select sum( coalesce(l.credit,0) ) - sum( coalesce(l.debit, 0) ) from account_move_line l inner join account_account a on l.account_id = a.id where a.code ilike '1%' and l.date between '" + str(date_from1) + "' and '" + str(date_to1) + "' and l.company_id = " + str(company_id.id) )
			...
			self._cr.execute("select sum( coalesce(l.credit,0) ) - sum( coalesce(l.debit, 0) ) from account_move_line l inner join account_account a on l.account_id = a.id where a.code ilike '1%' and l.date between '" + str(date_from2) + "' and '" + str(date_to2) + "' and l.company_id = " + str(company_id.id) )
