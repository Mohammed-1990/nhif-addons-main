...
class CompareRevenuesReport(models.AbstractModel):
	_name = 'report.kamil_accounting_reports.compare_revenues_template'
	...
	def _get_report_values(self, docids, data=None):
		...
		for account in all_accounts:
			...
			if found :
				...
				self._cr.execute("select sum(COALESCE( credit, 0 ) ) - sum(COALESCE( debit, 0 ) )  from account_move_line where account_id="  + str(account.id) + " AND date >= '" + str(date_from1) + "'  AND date <=  '" + str(date_to1) +  "'  " )

				actual_expense1 = self.env.cr.fetchone()[0] or 0.0


				self._cr.execute("select sum(COALESCE( credit, 0 ) ) - sum(COALESCE( debit, 0 ) )  from account_move_line where account_id="  + str(account.id) + " AND date >= '" + str(date_from2) + "'  AND date <=  '" + str(date_to2) +  "'  " )
