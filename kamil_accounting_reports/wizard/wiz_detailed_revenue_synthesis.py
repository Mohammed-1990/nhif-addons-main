...
class DetailedRevenuesSythesis(models.TransientModel):
	_name ='detailed.revenues.synthesis.wizard'

	group_ids = fields.Many2many('account.analytic.account',string='Groups', domain="[('code','=ilike','1%')]")
	year = fields.Char('Year', default=str(fields.Date.today().year), required=True)
	# company_ids = fields.Many2many('res.company', string='Companys')



	account_id = fields.Many2one('account.account', domain=lambda self:self.get_revenue_accounts(), string='Select Group', required=True)
	...
	def get_revenue_accounts(self):
		...
		for account_id in self.env['account.account'].search([('code','=ilike','1%'),('is_group','=','group')]):
			if len(account_id.code) == 3 or len(account_id.code) == 1 :
	...
	def get_report(self):
		...
		return self.env.ref('kamil_accounting_reports.detail_rev_report').report_action(self, data=data)
