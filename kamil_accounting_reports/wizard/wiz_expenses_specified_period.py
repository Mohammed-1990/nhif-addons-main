...
class ExpensesSpecifiedPeriodWizard(models.TransientModel):
	_name ='expenses.specified.period.wizard'
	...
	expenses_account_ids = fields.Many2many("account.analytic.account",domain=lambda self:self.get_budget_domain())
	...
	def get_budget_domain(self):
		# budget_item_ids =  self.env['account.analytic.account'].search([('code','=ilike','1%')])._ids
		
		budget_item_ids =  self.env['account.analytic.account'].search(['|','|',('code','=ilike','2%'),('code','=ilike','3%'),('code','=ilike','4%')])

		all_ids = []
		all_codes = []
		for item in budget_item_ids:
	...
	def get_report(self):
		...
		data = {
			...
			'from': {
				...
				'expenses_account_ids' : self.expenses_account_ids.ids,
				'type' : self.type,
		...
		return self.env.ref('kamil_accounting_reports.expenses_specified_period_report').report_action(self, data=data)
