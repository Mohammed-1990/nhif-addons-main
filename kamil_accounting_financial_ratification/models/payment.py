...
class Payments(models.Model):
	...
	ratification_name = fields.Text(related='ratification_id.name',string='Description')
	...
	state = fields.Selection([('draft','Draft'),('confirmed','Confirmed'),('approved','Audited'),('canceled','Canceled')], default='draft',track_visibility='always')
	...
	def get_journal_balance(self):
		...
		if self.journal_id:
			...
			if self.state == 'draft':
				self.env.user.sudo().notify_info( str_balance )
	...
	def onchange_journal(self):
		for record in self:
			...
			if record.journal_id:
				self._cr.execute("select sum(COALESCE( debit, 0 ))-sum(COALESCE( credit, 0 )) from account_move_line where account_id="  + str(record.journal_id.default_credit_account_id.id) + " AND date <= '" + str(date_to) + "'  " )
				balance = self.env.cr.fetchone()[0] or 0.0
				...
				if record.state == 'draft':
					...
					# warning_mess = {
					# 	'title': _('الرصيد المتاح'),
					# 	'message' : _( 'الرصيد المتاح ' + str_balance ),
					# }
		...
		for record in self:
			...
			if record.ratification_id:
				...
				for line in record.ratification_id.line_ids:		
					...
					self._cr.execute(
					""" 
						INSERT INTO payment_ratification_line(partner_id,name,analytic_account_id,account_id,cost_center_id,amount,branch_id,ratification_type,accounts_receivable_types,accounts_payable_types,account_code,ratification_id,company_id,create_uid,ratification_line_id) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)	
					""", ( (line.partner_id.id or None) , (str(line.name) or None), (line.analytic_account_id.id or None) ,(line.account_id.id or None), (line.cost_center_id.id or None), (line.amount or None),(line.branch_id.id or None),(line.ratification_type or None),(line.accounts_receivable_types or None),(line.accounts_payable_types or None),(line.account_code or None),record.ratification_id.id,self.env.user.company_id.id,self.env.user.id,line.id))
	...
	def create(self, vals):
		...
		for other_payment in self.env['ratification.payment'].search([('ratification_id','=',self._context.get('default_ratification_id', 0) )]):

			raise ValidationError(_('Payment already created for this Ratification'))
...
class PaymentRatificationLine(models.Model):
	...
	company_id = fields.Many2one('res.company', default= lambda self:self.env.user.company_id.id)
	ratification_line_id = fields.Char()
...
class CancelPaymenWizard(models.TransientModel):
	...
	reverse_or_reset = fields.Selection([('reverse','Reverse Entry'),('reset','Reset to Draft')], default='reverse')
	date = fields.Date('Reverse Entry Date',default=lambda self: fields.Date.today() )
	...
	def do_cancel(self):
		...
		for account_move in self.env['account.move'].search([('ratification_payment_id','=', self.env.context.get('payment_id',0))]):
			...
			if self.reverse_or_reset == 'reverse':
				
				copy_move = account_move.copy()
				
				copy_move.date = self.date
				copy_move.name = _('عكس القيد - رقم المستند -- ' + str(account_move.document_number) + ' --> ') + str(copy_move.name)
				copy_move.ref = _('عكس القيد - رقم المستند -- ' + str(account_move.document_number) + ' --> ') + str(copy_move.ref)
				...
				for line in account_move.line_ids:
					...
					lines.append( (0,0,{
						'account_id' : line.account_id.id,
						'partner_id' : line.partner_id.id,
						'name' :  _('عكس القيد - رقم المستند -- ' + str(account_move.document_number) + ' --> ') + line.name,
				...
				for payment in self.env['ratification.payment'].search([('id','=',  self.env.context.get('payment_id',0) )]):
					...
					payment.ratification_id.is_locked = True

			if self.reverse_or_reset == 'reset':
				account_move.button_cancel()
				account_move.unlink()

				for payment in self.env['ratification.payment'].search([('id','=',  self.env.context.get('payment_id',0) )]):
					payment.state = 'draft'
					payment.ratification_id.state = 'payment_created'
