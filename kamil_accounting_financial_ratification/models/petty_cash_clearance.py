...
class Clearance(models.Model):
	...
	clearance_date = fields.Date(default=lambda self: fields.Date.today(),track_visibility='always')
	...
	clipboard_number = fields.Char(string='Clipboard Number/39 Receipt Number',track_visibility='always', default='-')
	...
	def do_clear(self):
		for record in self:
			...
			move_id = self.env['account.move'].create({
				'name' : record.name,
				'date' : record.clearance_date,
				'journal_id': record.journal_id.id,
				'company_id' : record.branch_id.id,
				'petty_cash_clearance_id' : record.id,
				'document_number' : record.clipboard_number,
				})

			move_lines = []
			if lines_net_amount > 0:
