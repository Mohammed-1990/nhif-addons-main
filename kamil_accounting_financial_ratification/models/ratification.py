...
class Ratification(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string='Payee',track_visibility='always', domain= lambda self:self.get_branch_partner() )
    ...
    date = fields.Date(string='Date',default=lambda self: fields.Date.today(),track_visibility='always')

    ratification_list_id = fields.Many2one('ratification.list',string='Ratification List',track_visibility='always')
    ...
    state = fields.Selection([('draft','Draft'),('executed','Under Execution'),('audited','Approved'),('payment_created','Payment Created'),('payment_confirmed','Payment Confirmed'),('merging_completed','Merging completed'),('paid','Paid'),('canceled','Canceled'),('send_to_ia','Send to Internal Auditor')], default='draft',track_visibility='always')
    ...
    service_provider_loan_start_date = fields.Date(default=lambda self: fields.Date.today())
    ...
    prepaid_expenses_start_date = fields.Date(default=lambda self: fields.Date.today())
    ...
    discount_type = fields.Selection([('percent', 'Percentage'), ('fixed', 'Fixed Amount')], default='fixed')

    percent = fields.Float(string="Percent", track_visibility='always')
    fixed = fields.Float(string="Fixed Amount", track_visibility='always')
    global_discount_amount = fields.Float(compute='compute_global_discount_amount',string="Discount Amount")
    ...
    def unlink(self):
        for record in self:
            ...
            for payment in self.env['ratification.payment'].search([('ratification_id','=',record.id)]):
                raise ValidationError(_('لا يمكنك حذف تصديق بعد تحرير الدفعية!'))
...
#############################################################
    ...
    def show_payments(self):
        ...
        return {
            ...
            'res_model' : 'ratification.payment',
            'domain' : [('ratification_id','=', self.id )],
            'context' : {'default_ratification_id':self.id,'default_date':self.date},
        }
    ...
    def do_cancel(self):
        for record in self:
            if record.ratification_list_id:
                record.ratification_list_id.ratification_ids = False
    ...
    def do_reset_to_draft(self):
        for record in self:
            for payment in self.env['ratification.payment'].search([('ratification_id','=',record.id),('state','=','draft')]):
                payment.unlink()
        self.state = 'draft'
        ...
        for record in self:
            ...
            for line in record.line_ids:
                if not line.account_id:
                    raise ValidationError(_('Please Add Account in Details'))
                if line.the_type == 'budget':
    ...
    state_change = fields.Char(compute='onchange_state_change')


    @api.multi
    @api.depends('state')
    def onchange_state_change(self):
        for record in self:

            one_percent_tax_found = False
            for line in record.tax_ids:
                if line.tax_id.amount_type == 'percent' and line.tax_id.amount == 1:
                    one_percent_tax_found = True

            if not one_percent_tax_found and record.state in ['executed']:
                warning_mess = {
                    'title': _('تنبيه ضريبة ال1%'),
                    'message' : _('لم تقم باضافة ضريبة ال1%'),
                }
                self.env.user.sudo().notify_info( warning_mess['message'] )
    ...
    @api.multi
    @api.depends('tax_ids','global_discount_amount','fixed')
    def compute_tax_ids(self):
        for record in self:
            ...
            if record.global_discount_amount > 0.0 :
                record.total_taxes_amount = total + record.global_discount_amount
            if  record.fixed > 0.0:
                record.total_taxes_amount = total + record.fixed
            else:
    ...
    @api.multi
    @api.depends('line_ids','loan_ids','tax_ids','global_discount_amount','fixed')
    def compute_net_amount(self):
        for record in self:
            ...
            net = total - total_taxes - total_loans - record.global_discount_amount - record.fixed
    ...
    def compute_global_discount_amount(self):
        for record in self:
            if record.discount_type =='percent':
                record.global_discount_amount = record.amount * (record.percent / 100.0)
    ...
    def onchange_partner(self):
        ...
        for record in self:
            ...
            if not found_ones:
                ...
                self.env.user.sudo().notify_danger(message=_( record.partner_id.name + ' Has Loans'))
    ...
    def create(self, vals):
        ...
        if create_id.date:
            ...
            one_percent_tax_found = False
            for line in create_id.tax_ids:
                if line.tax_id.amount_type == 'percent' and line.tax_id.amount == 1:
                    one_percent_tax_found = True

            if not one_percent_tax_found:
                warning_mess = {
                    'title': _('تنبيه ضريبة ال1%'),
                    'message' : _('لم تقم باضافة ضريبة ال1%'),
                }
...
class RatificationLine(models.Model):
    ...
    account_id = fields.Many2one('account.account', string='Account', domain=['|','|',('code','=ilike','2%'),('code','=ilike','3%'),('code','=ilike','4%'),('is_group','=','sub_account')])
    ...
    @api.multi
    @api.depends('ratification_id', 'ratification_list_id')
    def _compute_date(self):
        for record in self:
            if record.ratification_id and record.ratification_id.date:
                record.date = record.ratification_id.date
            elif record.ratification_list_id and record.ratification_list_id.date:
                record.date = record.ratification_list_id.date
            else:
                record.date = fields.date.today()  # Set to today's date if neither is available


    @api.multi
    @api.onchange('account_id')
    def onchange_account_to_get_pettycash(self):
        for record in self:
            if record.account_id:
                if record.account_id.is_petty_cash_account:
                    record.accounts_receivable_types = 'petty_cash'
                    record.ratification_type = 'petty_cash'
        ...
        for record in self:
            if record.account_id:
                if record.date:
                    ...
                    desired_budget = self.env['crossovered.budget'].search([('state','=','validate'),('date_from','<=',record.date),('date_to','>=',record.date)])
                else :
                    desired_budget = self.env['crossovered.budget'].search([('state','=','validate')],limit=1)
                for budget in desired_budget :
    ...
    def onchange_account_or_partner(self):
        for record in self:
            if record.account_id:
                if record.account_id.is_petty_cash_account or record.account_id.is_loan_account:
                    the_partner_id = record.partner_id.id or record.ratification_id.partner_id.id
                    the_partner_name = record.partner_id.name or record.ratification_id.partner_id.name
                    ...
                    if the_partner_id:
                        ...
                        if record.account_id.is_petty_cash_account:
                            ...
                            account_type = 'عهد'
                            account_ids = self.env['account.account'].search([('is_petty_cash_account','=',True)])._ids

                            in_condition = " in "
                            if len(account_ids) == 1:
                                in_condition = " = " + str(record.account_id.id)
                            else:
                                in_condition = " in " + str(tuple(account_ids) )

                            self._cr.execute("select sum(COALESCE( debit, 0 ) ) - sum(COALESCE( credit, 0 ) )  from account_move_line where account_id  "  + in_condition + " AND partner_id = " + str(the_partner_id) + " " )
                            amount = self.env.cr.fetchone()[0] or 0.0

                        if record.account_id.is_loan_account:

                            account_type = 'سلف'

                            account_ids = self.env['account.account'].search([('is_loan_account','=',True)])._ids
                        ...
                        if amount > 0:
                            warning_mess = {
                                'title': _('تنبيه ب' + account_type + ' سابقة!'),
                                'message' : _( str(the_partner_name) + ' لديه(ا) '+ account_type +' سابقة بمبلغ  ' + str( '{:,.2f}'.format( amount )  ) ),
                            }
                            return {'warning': warning_mess}
    ...
    @api.multi
    @api.onchange('amount')
    def onchange_amount_2(self):
        for record in self:
            if record.the_type == 'budget':
                executed_amount = record.approved_value - record.remaining_value
                if record.amount > 0 :
                    if record.approved_value > 0:
                        percentage = (executed_amount + record.amount ) / record.approved_value * 100
                    else:
                        percentage = 0
                    ...
                    if percentage > 80:
                        ...
                        warning_mess = {
                            'title': _('تنبيه صرف اكثر من 80% '),
                            'message' : _( 'نسبة الصرف على بند ' + str(record.account_id.name) + ' ستصبح ' + str( '{:,.2f}'.format( percentage )  ) + '%' ),
                        }
                        self.env.user.sudo().notify_warning( warning_mess['message'] )
                        return {'warning' : warning_mess}
...
class RatificationPrepaidExpenses(models.Model):
    ...
    transfer_date = fields.Date(track_visibility='always', default=lambda self: fields.Date.today())
...
class Account(models.Model):
    ...
    is_collection_account = fields.Boolean()
    is_service_center_account = fields.Boolean()
