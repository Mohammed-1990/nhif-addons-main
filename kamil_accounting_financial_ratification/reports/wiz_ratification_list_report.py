...
class RatificationListReport(models.AbstractModel):
	...
	def _get_report_values(self, docids, data=None):
		...
		if list_type == 'ratification_list':
			...
			for rat_list in self.env['ratification.list'].sudo().search([('id','=',list_id)]):

				description = rat_list.name
				create_uid = rat_list.sudo().create_uid.name
				x_create_user_id = rat_list.x_create_user_id.name

				total_net_amount_in_words = rat_list.total_net_amount_in_words

				for payment in self.env['ratification.payment'].sudo().search([('ratification_id.ratification_list_id','=',rat_list.id)]):
					payment_number = payment.code
					cheque_number = payment.check_number
					payment_date = payment.date

				for line in rat_list.sudo().ratification_line_ids:
					found = False
					for raw in docs:
					...
					if not found:
						total_deductions = 0
						for deduction_line in rat_list.sudo().other_deduction_ids:
						...
						docs.append({
							'partner_id' : line.sudo().partner_id.id,
							'partner_name' : line.sudo().partner_id.name,
							'amount' : line.amount,
				...
				if rat_list.is_complex:
					docs = []
					for line in rat_list.sudo().partners_net_ids:
							docs.append({
							'partner_id' : line.sudo().partner_id.id,
							'partner_name' : line.sudo().partner_id.name,
							'amount' : 0,
							'deduction_amount' : 0,
							'account_number' : line.partner_id.sudo().account_number,
							'bank_name' : line.sudo().partner_id.bank_name,
							'bank_branch_name' :
							line.sudo().partner_id.bank_branch_name, 
