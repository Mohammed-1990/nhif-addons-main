...
class payrollreportexcelwiz(models.TransientModel):
    ...
    from_date = fields.Date('From Date', required=True, default=lambda self: fields.Date.to_string(date.today().replace(day=1)))
    date_end= fields.Date('To Date', required=True, default=lambda self: fields.Date.to_string((datetime.now() + relativedelta(months=+1, day=1, days=-1)).date()))
    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            worksheet.write(row, 0, _('Employee'), cell_text_format)
            worksheet.write(row, 1, _('Account Number'), cell_text_format)
            ...
            payslip_ids=self.env['hr.payslip'].search([('date_from','=',self.from_date),('date_to','=',self.date_end),('branch_id','child_of',[self.company.id])])
            if payslip_ids:

                for payslip in payslip_ids:
                    name = payslip.employee_id.name
                    id = payslip.employee_id.partner_id.account_number
