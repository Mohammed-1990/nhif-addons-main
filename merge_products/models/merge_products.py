...
class MergeProducts(models.Model):
    ...
    date = fields.Date(default=lambda self: fields.Date.to_string(date.today().replace(day=1)))
    @api.model
        ...
        for line in self.product_line_ids:
            ...
            for model in model_dict:
                ...
                if model_search_ids:
                    ...
                    if str(Model) == 'stock.move.line()':
                        states = []
                        for move in model_search_ids:
            ...
            line.remove_product_id.toggle_active()
            line.remove_product_id.product_tmpl_id.toggle_active()
...
class MergeProductLine(models.Model):
    ...
    remove_product_id = fields.Many2one('product.product', 'First Product', domain="[('id', '!=', new_product_id)]", copy=False, required=True)
    remove_product_qty = fields.Float(related='remove_product_id.qty_available')
    ...
    new_product_qty = fields.Float(related='new_product_id.qty_available')
