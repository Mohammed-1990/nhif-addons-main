...
class ExpensesBudgetItemsReport(models.AbstractModel):
	_name = 'report.kamil_accounting_budget.expenses_budget_items_template'
		...
		for account in all_accounts:
			...
			for company_id in company_ids:
				...
				if account_id:
					...
					self._cr.execute("select sum(COALESCE( debit, 0 ) ) - sum(COALESCE( credit, 0 ) )  from account_move_line where account_id="  + str(account_id.id) + " AND date >= '" + str(date_from) + "'  AND date <=  '" + str(date_to) +  "' and company_id = " + str(company_id) )
					...
					if annual_budget_value > 0:
						remaining_value = annual_budget_value - abs(actual_value)
			...
			if account_annual_budget_value:
