# -*- coding:utf-8 -*-
from odoo import models, fields, api
from datetime import datetime

class RequestFilter(models.TransientModel):
	_name = 'request.filter'
	_description = 'Request Filter'

	request_ids = fields.Many2many('purchase.request', string='Requests')
	type = fields.Selection([
						('public_tender','Public Tender'),
						('limited_tender','Limited Tender'),
						('request_for_quotaion','Request For Quotaion(RFQs)'),
						('direct_purchase','Direct Purchase')
						],
						'Type')
	@api.multi
	def _domain_request(self):
		context = self._context
		
		current_uid = context.get('uid')
		user = self.env['res.users'].browse(current_uid)
		if user.company_id.is_main_company:
			return [('type','=','request_for_quotaion'),('state','=','in_process')]

		else:
			return [('type','=','request_for_quotaion'),('state','=','in_process'),('is_transfer','=','1')]


	@api.model
	def default_get(self, default_fields):
		res = super(RequestFilter, self).default_get(default_fields)
		type = ''
		model = self.env.context.get('active_model')
		if model == 'purchase.requisition':
			request = self.env['purchase.requisition'].browse(self.env.context.get('active_id'))
			type = request.type
		if model == 'purchase.rfq':
			request = self.env['purchase.rfq'].browse(self.env.context.get('active_id'))
			type = request.type

		res['type'] = type
		return res



	def collecting(self):
		data = []
		product_list = []
		if self._context.get('active_model') == 'purchase.requisition':
			
			order = self.env['purchase.requisition'].browse(self._context.get('active_id'))
			order_lind = self.env['purchase.requisition.line']
			for rec in self.request_ids:
				for line in rec.line_ids:
					if not any(line.product_id.id == l['product'] for l in product_list):
						product_list.append({'product':line.product_id.id,'product_uom_id':line.product_id.uom_po_id.id})

			for x in product_list:
				qty = 0.0
				for rec in self.request_ids:
					for line in rec.line_ids:
						if line.product_id.id == x['product']:
							qty += line.product_qty
				data.append((0,0,{
						'product_id':x['product'],
						'product_qty':qty,
						'product_uom_id':x['product_uom_id']
					}))

			order.line_ids.unlink()
			order.write({'line_ids':data, 'request_ids':[(6,0, [v.id for v in self.request_ids])]})

		if self._context.get('active_model') == 'purchase.rfq':
			order = self.env['purchase.rfq'].browse(self._context.get('active_id'))
			order_lind = self.env['purchase.rfq.line']
			for rec in self.request_ids:
				for line in rec.line_ids:
					if not any(line.product_id.id == l['product'] for l in product_list):
						product_list.append({'product':line.product_id.id,'product_uom_id':line.product_id.uom_po_id.id})

			for x in product_list:
				qty = 0.0
				for rec in self.request_ids:
					for line in rec.line_ids:
						if line.product_id.id == x['product']:
							qty += line.product_qty
				data.append((0,0,{
						'product_id':x['product'],
						'product_qty':qty,
						'product_uom_id':x['product_uom_id']
					}))

			order.line_ids.unlink()
			order.write({'line_ids':data, 'request_ids':[(6,0, [v.id for v in self.request_ids])]})
		

		