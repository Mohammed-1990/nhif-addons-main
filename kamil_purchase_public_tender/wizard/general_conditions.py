...
class WizGeneralConsitionsBook(models.TransientModel):

	_name = 'wiz.order.general.conditions'
	_description = 'Wiz order general conditions'

	order_id = fields.Many2one('purchase.order',string='tender')
	condtions_ids = fields.Many2many('pr.general.conditions','wiz_order_general_conditon_rel')
	select_all = fields.Boolean('Select All')

	@api.model
	def default_get(self, default_fields):
		res = super(WizGeneralConsitionsBook, self).default_get(default_fields)
		vals = []
		book = self.env['purchase.order'].browse(self.env.context.get('active_ids'))
