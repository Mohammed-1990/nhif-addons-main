...
class PurchaseReport(models.Model):
	...
	requisition_id = fields.Many2one('purchase.requisition', string='# Tender', readonly=True)
	price_total = fields.Float('Total Price', readonly=True)
	order_id = fields.Many2one('purchase.order','Books')
		...
		select_str = """
			WITH currency_rate as (%s)
				SELECT
					...
					sum(l.product_qty) as unit_quantity,
	...
	def _from(self):
		from_str = """
			purchase_order_line l
				...
				join purchase_requisition pr on (s.requisition_id=pr.id)
